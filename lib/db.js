...
const service = {
  ...
  getAdminUsers: () => {
    let promise = new Promise((resolve, reject) => {
      service.client.sinter('users','admin-users', (err, data) => {
        if (err) { 
          reject(err) 
        } else {
          resolve(data);
  ...
  canRemind: (userId) => { 
    let promise = new Promise((resolve, reject) => {
      Promise.all([service.getMarkAsAppearedToday(userId), service.getMarkAsReminded(userId), service.getUserCheckIn(userId)])
      .then((results) => {
        let agree = false;
        let diff = Math.floor((Date.now() - results[0]) / 60000)

        if (!results[1] && !results[2] && diff >= 15) {
          agree = true;
        }
  ...
  resetAppearedToday: (userId) => {
    let promise = new Promise((resolve, reject) => {
      service.client.set(userId + '-appeared_today', 0, (err, data) => {
        resolve(data);
      });
    })
  ...
  getUserHours: (userId, keys) => {
    let promise = new Promise((resolve, reject) => {
      service.client.hmget(userId + '-payroll', keys, (err, data) => {
        let userHours = {userId: userId, days: Number(data[0]), hours: Number(data[1])};
        resolve(userHours);
      });
    })
