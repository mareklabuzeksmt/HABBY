  ...
  canRemind: (userId) => { 
    let promise = new Promise((resolve, reject) => {
      ...
      .then((results) => {
        ...
        console.log(userId, 'last call', new Date(Date.now() + cronInterval), new Date(lastCall.getTime()))

        if (results[1] == 0 && userWasReminded == 0 && userWasOffline == 1 && isUserLoggedIn == 1
          && ((diff >= timeoutValue) || (Date.now() + cronInterval >= lastCall.getTime()))
        ) {
          agree = true;
        }
