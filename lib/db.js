  ...
  canRemind: (userId) => { 
    let promise = new Promise((resolve, reject) => {
      ...
      .then((results) => {
        ...
        let userWasReminded = results[1]
        let userHasCheckIn = results[2]
        let userWasOffline = results[3]
        let appearedToday = results[0]

        console.log(userId, 'getMarkAsReminded', userWasReminded == 0)
        ...
        if (Boolean(appearedToday) == 1 && userWasReminded == 0 && userHasCheckIn == 0 && isUserLoggedIn == 1
